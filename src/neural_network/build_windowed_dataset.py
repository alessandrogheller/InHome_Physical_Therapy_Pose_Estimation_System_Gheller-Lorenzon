"""
Build the train/validation dataset from normalized MMFi pose sequences.

Each sliding window receives an exercise class and the subject-level quality
score generated by generate_quality_labels.py, then is saved as a compressed
NPZ file for the GRU and TCN training scripts.
"""
import os
import sys
import csv
...
def load_raw_sequence(database, subject, action_code):
    """Load one subject/action sequence from the MMFi RGB modality."""
    try:
...
def main():
    # Load the quality labels first because every usable sequence needs both
    # pose data and a corresponding score.
    ...
    # Split complete subjects, rather than individual windows, to prevent frames
    # from the same person appearing in both training and validation data.
    ...
    # Build windows for every exercise and subject, assigning each one to the
    # split determined above.
    for class_idx, (action_name, action_code) in enumerate(ACTIONS.items()):
        ...
        for subject in subjects:
            ...
            # Normalize the pose sequence before flattening each frame to the
            # fixed feature representation expected by the neural networks.
            ...
            # Add a horizontally mirrored copy as extra training data.
            # Side-specific exercises are assigned to the opposite class.
            ...
            # Store the original and mirrored windows together with their class
            # and quality targets for the selected split.
    ...
    # Concatenate all collected windows into the arrays written to disk.
    save_kwargs = {}
    ...
    # Keep identical copies for the GRU and TCN pipelines.
